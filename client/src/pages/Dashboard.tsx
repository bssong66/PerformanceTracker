...
export default function Dashboard() {
  ...
  const eventStats = {
    ...
    completedEvents: Array.isArray(events) ? (events as any[]).filter((e: any) => e.completed).length : 0,
    todayEvents: Array.isArray(events) ? (events as any[]).filter((e: any) => {
      const eventDate = format(new Date(e.startDate), 'yyyy-MM-dd');
      return eventDate === todayStr;
    }) : [],
    weekEvents: Array.isArray(events) ? (events as any[]).filter((e: any) => {
      const eventDate = new Date(e.startDate);
      return eventDate >= weekStart && eventDate <= weekEnd;
    }) : [],
    upcomingEvents: Array.isArray(events) ? (events as any[]).filter((e: any) => {
      const eventDate = new Date(e.startDate);
  ...
  const eventPriorityStats = {
    high: {
      total: Array.isArray(events) ? (events as any[]).filter((e: any) => e.priority === 'high').length : 0,
      completed: Array.isArray(events) ? (events as any[]).filter((e: any) => e.priority === 'high' && e.completed).length : 0,
    },
    medium: {
      total: Array.isArray(events) ? (events as any[]).filter((e: any) => e.priority === 'medium').length : 0,
      completed: Array.isArray(events) ? (events as any[]).filter((e: any) => e.priority === 'medium' && e.completed).length : 0,
    },
    low: {
      total: Array.isArray(events) ? (events as any[]).filter((e: any) => e.priority === 'low').length : 0,
      completed: Array.isArray(events) ? (events as any[]).filter((e: any) => e.priority === 'low' && e.completed).length : 0,
    },
  };
          ...
          <CardContent>
            <div className="grid grid-cols-7 gap-2">
              {weekDays.map((day, index) => {
                ...
                // 해당일의 일정
                const dayEvents = Array.isArray(events) ? (events as any[]).filter((event: any) => 
